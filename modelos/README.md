...
1. Abre `herramientas/exportar-glb.html` en el navegador
...
4. Ponla en `modelos/glb/` y **agrega su nombre a
   `glb/indice.json`**:
   ```json
...
- **Tokens del sistema** (`token('--talavera-500')`) — se leen de
  `design-system.css` en vivo. Si cambia la paleta, esta cocina se
  repinta sola. No es un detalle: el juego nació con la paleta
  anterior, y cuando el sistema de diseño cambió, el `git merge` no
  vio ningún conflicto — sin esto habría quedado con los colores de
