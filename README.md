...
La **preparación de la fanesca**, hecha con los dedos. Nada de
arrastrar cartas — aquí se desgrana, se desvaina, se aprieta, se corta
y se frota, al estilo de esos juegos de reventar burbujas en los que
el gusto está en el gesto.

Jugable en: https://franciscombp.github.io/fanesca/

Se instala como app y funciona sin conexión. No tiene build, ni
dependencias que bajar: HTML, CSS y módulos ES servidos tal cual,
con Three.js versionado en `vendor/`.
...
(crema y azulejo celeste) y cuando el sistema de diseño pasó a la del
barrio —talavera, rosa mexicano, peltre— el `git merge` no vio ningún
...
| `design-system.css` | Los tokens: paleta, tipografía, sombras, y los componentes (`.btn`, `.panel`, `.sheet`). |
| `icons.js` | Las ilustraciones planas de las pantallas de papel. Lo del nivel es 3D y vive en `modelos/`. |
| `sw.js`, `version.js`, `actualizador.js` | Que se instale, funcione sin conexión y avise cuando hay versión nueva. Subir `APP_VERSION` **es** publicar. |
...
Necesita un servidor: los módulos ES y el service worker no cargan
desde `file://`.

```bash
npx serve .        # y abrir la raíz
```

## Publicarlo

`.github/workflows/pages.yml` publica en GitHub Pages en cada push a
